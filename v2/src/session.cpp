...
MessageWriter Session::GetMessageWriter()
{
    return MessageWriter(socket_ptr_);
}
...
void Session::MessageLoop()
{
    ...
    socket_ptr_->async_read_some(asio::buffer(buffer_ + read_size_,
                                              buffer_size_ - read_size_),
